...
edgelen是一个 通过图片识别出周长的 java库。纯Java实现，没有使用任何第三方库或框架。可以在任何支持Java的程序中使用。比如可以封装在Android APP中。
...
聚类后效果如下，为了减少内存使用，背景只读取上下左右指定宽度的边框。

![back_cluster](images/back_cluster.jpg)
...
目前实现了层次聚类和密度聚类两种方式，速度及性能上层次聚类都稍差，有优化的空间
